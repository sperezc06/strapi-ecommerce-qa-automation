...
export default function FormCheckoutShippingService({
  ...
  const getCountry = () => {
    return countryList.find(
      (item) => formShippingInformation.country === item.code
    )?.name;
  };
  ...
  const { mutate: mutateCheckoutItem, isLoading: loadingCheckout } =
    useMutation(checkoutItem, {
      onSuccess: (data) => {
        // If URL is a Stripe checkout URL, redirect to it
        // If URL is a local transaction page, redirect to it (user will see Pay Now button)
        if (data.url) {
  ...
  // Selecting shipping service and set to state management
  ...
  // Validate if the shipping service selected. then hit api checkout
  const goToPayment = () => {
    ...
    if (isLoading) {
      ...
      if (shippingRate && shippingRate.rates.length <= 0) {
          ...
          <div className="flex h-[250px] mb-10 shrink-0 items-center justify-center rounded-md border border-dashed">
            <div className="mx-auto flex max-w-[420px] flex-col items-center justify-center text-center">
              ...
              <p className="mb-4 mt-2 text-sm text-muted-foreground">
                Oops, it seems like that are now available shipping service for
                the address or the parcel. please contact our team
              </p>
            </div>
          </div>
        );
        ...
        return (
          <>
            {shippingRate &&
              shippingRate?.rates?.map((item) => {
                return (
                  <Card
                    key={"rate-id-" + item.id}
                    onClick={() => selectShippingService(item)}
                    className={
                      item.id === formShippingService.id
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 text-sm">
                          ...
                          {item.est_delivery_days &&
                            `Est. (${item.est_delivery_days} Days)`}
                        </p>
                      </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-between">
              ...
              <div>
                ...
                <div className="flex flex-col">
                  <p className="text-sm text-right mr-5">
                    {formShippingInformation.email}
                  </p>
                  <p className="text-sm text-right mr-5">
                    {formShippingInformation.phone_number}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <div>
                ...
                <div className="flex flex-col">
                  <p className="text-sm text-right mr-5">
                    {formShippingInformation.city},{" "}
                    {formShippingInformation.state}, {getCountry()}
                  </p>
                  <p className="text-sm text-right mr-5">
                    {formShippingInformation.zip_code}
                  </p>
                </div>
              </div>
            </div>

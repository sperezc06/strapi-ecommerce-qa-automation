            ...
            {dataTransaction.payment.payment_status === "UNPAID" &&
              dataTransaction.payment.url && (
                <Button 
                  className="w-full" 
                  onClick={() => {
                    // If URL is a local transaction page (no Stripe), redirect to success page
                    if (dataTransaction.payment.url?.includes('/transaction/')) {
                      const url = new URL(dataTransaction.payment.url, window.location.origin);
                      const orderId = url.pathname.split('/transaction/')[1]?.split('?')[0];
                      const secret = url.searchParams.get('secret');
                      if (orderId && secret) {
                        window.location.href = `/payment/success?order_id=${orderId}&secret=${secret}`;
                      }
                    } else {
                      // Stripe URL - open in same window
                      window.location.href = dataTransaction.payment.url;
                    }
                  }}
                >
                  Pay Now
